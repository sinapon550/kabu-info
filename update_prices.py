...
COMPANIES = [
    ...
    ("6861.T", "キーエンス"),
    ("6324.T", "ハーモニック"),
    # テーマ株ウォッチ（需要が構造的に確実な「国策」テーマ）
    ("4062.T", "イビデン"),        # 半導体基板
    ("5803.T", "フジクラ"),        # 電線・DC向け光配線
    ("5802.T", "住友電工"),        # 電線・送電
    ("9502.T", "中部電力"),        # 電力
    ("6501.T", "日立製作所"),      # 重電・送配電
    ("3778.T", "さくらインターネット"),  # データセンター
    ("1942.T", "関電工"),          # 電気工事
    ("7011.T", "三菱重工業"),      # 防衛
    ("7012.T", "川崎重工業"),      # 防衛
    ("7013.T", "IHI"),             # 防衛
    ("4704.T", "トレンドマイクロ"),  # サイバーセキュリティ
    ("2326.T", "デジタルアーツ"),   # サイバーセキュリティ
    # 保有ETF（積立中）
    ("200A.T", "日経半導体ETF"),    # 日経半導体株指数連動・キオクシア等
    # 親の保有
    ("8306.T", "三菱UFJ"),
    # 個人の保有（株価のみ・ニュース対象外）
    ("2502.T", "アサヒグループHD"),
...
ALIASES = {
    ...
    "4062.T": ["ibiden", "イビデン"], "5803.T": ["fujikura", "フジクラ"],
    "5802.T": ["sumitomo electric", "住友電工", "住友電気"], "9502.T": ["chubu electric", "中部電力"],
    "6501.T": ["hitachi", "日立"], "3778.T": ["sakura internet", "さくらインターネット", "さくら"],
    "1942.T": ["kandenko", "関電工"], "7011.T": ["mitsubishi heavy", "三菱重工"],
    "7012.T": ["kawasaki heavy", "川崎重工"], "7013.T": ["ihi"],
    "4704.T": ["trend micro", "トレンドマイクロ"], "2326.T": ["digital arts", "デジタルアーツ"],
    "200A.T": ["日経半導体", "semiconductor", "キオクシア", "kioxia"],
